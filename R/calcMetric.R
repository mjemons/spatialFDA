...
#' @param r_seq the range of r values to compute the function over
#'
#' @return a list of spatstat metric objects with the fov number, the number of points and the centroid of the image
#' @export
#'
#' @examples
#'  spe <- imcdatasets::Damond_2019_Pancreas("spe", full_dataset = FALSE)
...
#' @import dplyr
calcMetricPerFov <- function(spe, selection, fun, marks = NULL, r_seq = NULL){
  fovs <- spe$image_number %>% as.list() %>% unique()
  metric_ls <- lapply(fovs, function(x){
    spe_sub <- subset(spe, ,image_number == x)
    metric_res <- extractMetric(spe = spe_sub, selection = selection, fov = x, fun = fun, marks = marks, r_seq = r_seq)
    return(metric_res)
  })
  return(metric_ls)
}
